...
class SaveScreen(Screen):
    def __init__(self, game):
        ...
        self.save1B = Button(self.game.window.root, text="Debug", command=self.save1,
                             bg="#%02x%02x%02x" % (255, 165, 0), font="Helvetica 15 bold", padx=10, pady=10)
        self.save1B.pack(in_=self.f, pady=25)

        self.save2B = Button(self.game.window.root, text="Final path", command=self.save2,
                             bg="#%02x%02x%02x" % (255, 165, 0), font="Helvetica 15 bold", padx=10, pady=10)
        self.save2B.pack(in_=self.f, pady=25)

        self.save3B = Button(self.game.window.root, text="Generate map", command=self.save3,
                             bg="#%02x%02x%02x" % (255, 165, 0), font="Helvetica 15 bold", padx=10, pady=10)
